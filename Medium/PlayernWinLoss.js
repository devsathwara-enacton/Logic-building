...
var findWinners = function (matches) {
  ...
  const notLostAny = Object.keys(wins).filter(
    (player) => losses[player] === undefined || losses[player] === 0
  );
  const lostOneMatch = Object.keys(losses).filter(
    (player) => losses[player] === 1
  );
  notLostAny.sort((a, b) => a - b);
  lostOneMatch.sort((a, b) => a - b);

  return [notLostAny.map(Number), lostOneMatch.map(Number)];
};
